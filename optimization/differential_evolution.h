#ifndef __ST_ALG_DIFFERENTIAL_EVOLUTION_H__
#define __ST_ALG_DIFFERENTIAL_EVOLUTION_H__
...
namespace stalg
{
    // Сделать трём индексам барбуз ! (барбУз)
    // Принимает три числа из отрезков [m, M], [m, M - 1], [m, M - 2], после барбуза они все три будут из отрезка [m, M] и будут все различными
    // Есть баг - барбузить надо так, чтобы a, b, c ещё не равнялись главному индексу - индексу обрабатываемого жука в популяции
    // 13 окт 2016 - баг поправили.
    // Функция барбузинга требует полноценного автоматического тестирования (в том числе стресс тест). Стресс тест
    // должен проверять, что после барбузинга все 4 числа будут различными.
    template<typename T>
    void Barbuse(const T index, T& a, T& b, T& c)
    {
        if (a >= index)
        {
            ++a;
        ...
        if ((c >= index && c >= a) || (c >= index && c >= b) || (c >= a && c >= b))
        {
            ++c;
        }
        if (c >= index && c >= a && c >= b)
    ...
    // Проверить, отбарбузились ли 4 числа.
    template<typename T>
    void CheckBarbusing(const T index, const T a, const T b, const T c)
    {
        if (index == a || index == b || index == c || a == b || a == c || b == c)
        {
            std::cout << "Bad barbusing : " << a << ", " << b << ", " << c << std::endl;
            throw std::runtime_error("Bad barbusing !!!");
        }
    }

    template<class ReturnType, class ArgumentType>
    class Optimizer_DifferentialEvolution : public basic_optimizer<ReturnType, ArgumentType>
    {
    public:
    ...
    private:
        // Параметры метода
        unsigned int max_steps_;                             // Количество шагов эволюции (поколений)
        double F_parameter_;                                 // Сила мутации
        double mutation_probability_;                        // Вероятность мутации
        //!!!!!!!!!!!!!! Следующий параметр, возможно, стоит заменить на параметр "плотность заселения" и считать динамически от размерности задачи
        unsigned int size_of_population_;                    // Численность популяции

    private:
        // Рабочие данные метода
        std::vector<FullArgumentType> population_;
        std::vector<FullArgumentType> new_population_;
    ...
    private:
        Optimizer_DifferentialEvolution() {}

    public:
        explicit Optimizer_DifferentialEvolution(    const unsigned int dimension
...
#endif // __ST_ALG_DIFFERENTIAL_EVOLUTION_H__
