...
template<typename T>
void Merge(T* const data, const int length, const int border)
{
    T* begin_first = data;
    T* begin_second = data + border;
    T* const end_first = data + border;
    T* const end_second = data + length;
...
template<typename T>
void MergeSort(T* const data, const int length)
{
    if (length <= 1) {
        return;
    } else {
...
template<typename T>
void BubbleSort(T* const data, const int length)
{
    bool flag = true;
...
template<typename T>
void CombSort(T* const data, const int length)
{
